...
public class CLM : MonoBehaviour
{
    ...
    public class LINE
    {
        ...
        //the speaker may not be displaying their actual name, instead show another one temporarily
        ...
        public LINE(string rawLine)
        {
            ...
            if(dialogueAndActions.Length == 3) //contains dialogue
            {
                ...
                //the speaker may be using a disguise name. Get the disguise name or the name
                if(speaker.Contains(" as "))//the speaker is speaking as someone else, using a false name such as ???
                {
                    string[] speakerAndDisplay = speaker.Split(new string[1] { " as " }, System.StringSplitOptions.None);
                    speaker = speakerAndDisplay[0]; // otherwise the as portion will be included in the speakers actual name
                    speakerDisplayName = speakerAndDisplay[1];
                }
                else
                {
                    speakerDisplayName = ""; //only change the display name if one is provided
                }

                //self is a shortcut for the actual name of the character
                if(speakerDisplayName == "self")
                {
        ...
        void SegmentDialogue(string dialogue)
        {
            ...
            for(int i = 0; i < parts.Length; i++)
            {
                ...
                if(isOdd)
                {
                    ...
                    i++; //increment to move past the command and to the next bit of dialogue
                }
        ...
        public class SEGMENT
        {
            ...
            IEnumerator Running()
            {
                ...
                for(int i = 0; i < parts.Length; i++)
                {
                    ...
                    //a character should only be shown if the character prefeb exists
                    if(line.speaker != "narrator" && !line.speaker.Contains("*"))
                    {
                        ...
                        //this is a valid character that can show up on the screen
                        if(character != null)
                        {
