...
{
    public static Dictionary<TextMeshProUGUI, TextArchitect> activeArchitects = new Dictionary<TextMeshProUGUI, TextArchitect>();
    ...
    IEnumerator Construction()
    {
       ...
       int cpf = charactersPerFrame;
        ...
        while(vis < max)
        {
            if(skip)
            {
                //allow skippiing by increasing the characters per frame and the speed of occurance
                speed = 1;
                cpf = charactersPerFrame < 5 ? 5 : charactersPerFrame + 3;
            }

            //reveals a certain number of characters per frame
